...
if (isset($_POST['subm'])){
    if(isset($_SESSION['log']) && $_SESSION['log'] == TRUE){
        ...
        $query_rez = "SELECT * FROM rezervacije WHERE apartma_id = $id AND (
            (zacetek_rez <= '$konec_rez' AND konec_rez >= '$zacetek_rez')
        )";
        $result_rez = mysqli_query($link, $query_rez);

        $max_ljudi = $_GET['ido'];
        $skupno = $st_otrok + $st_odraslih;
        if($skupno <= $max_ljudi){
            if(mysqli_num_rows($result_rez) > 0){
                echo "<p>Žal je apartma že rezerviran v tem terminu!</p>";
            }else{
            ...
            echo "Uspešno ste rezervirali apartma!";
            }
        }else{
